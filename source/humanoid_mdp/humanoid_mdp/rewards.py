...
def joint_torque_tiredness_penalty(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """Penalize joint torques on the articulation."""
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    torque = torch.linalg.norm((asset.data.applied_torque), dim=1)
    limit = asset.data.joint_effort_limits
    return torch.linalg.norm(torque / limit, dim=1)

def joint_power_penalty(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """Penalize joint power on the articulation."""
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    power = asset.data.applied_torque * asset.data.joint_vel
    return torch.where(power <= 0, power, 0).sum(dim=1)
...
env: ManagerBasedRLEnvCfg, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
    ...
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    return asset.data.root_lin_vel_b[:, 2].pow(2)
...
def ang_vel_xy_penalty(
    ...
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    return torch.linalg.norm((asset.data.root_ang_vel_b[:, :2]), dim=1)
...
def base_acceleration_penalty(
    ...
    """Penalize base linear acceleration"""
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    lin_acc = torch.linalg.norm(asset.data.body_lin_acc_w, dim=1)
    ang_acc = torch.linalg.norm(asset.data.body_ang_acc_w, dim=1)
...
def joint_position_limit_penalty(env: ManagerBasedRLEnvCfg, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """Penalize joint positions outside of limits on the articulation"""
    asset: Articulation = env.scene[asset_cfg.name]
    joint_pos = asset.data.joint_pos[:, asset_cfg.joint_ids]  # (num_envs, num_joints)
    # Get joint limits - assuming your tensor format [lower_limit, upper_limit]
    joint_limits = asset.data.joint_pos_limits[:, asset_cfg.joint_ids]  # (num_envs, num_joints, 2)
    lower_limits = joint_limits[:, :, 0]  # (num_envs, num_joints)
    upper_limits = joint_limits[:, :, 1]  # (num_envs, num_joints)
    outside_limits = (joint_pos < lower_limits) | (joint_pos > upper_limits)
    return (outside_limits).float().sum(dim=1)
