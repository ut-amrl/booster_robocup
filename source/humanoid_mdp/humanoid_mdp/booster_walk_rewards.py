...
def get_feet_contact(asset: Articulation, env: ManagerBasedRLEnvCfg, body_ids):
    ...
    feet_edge_relative_pos = (
        convert_to_torch(feet_edge_relative_pos)
        .unsqueeze(0)
        .unsqueeze(0)
        .expand(env.num_envs, 2, -1, -1)
        .to(device=env.device)
    )  # [num_envs, num_feet, num_points, 3]
    feet_pos = (
        asset.data.body_pos_w[:, body_ids, :]
        .unsqueeze(2)
        .expand(-1, -1, feet_edge_relative_pos.shape[2], -1)
        .reshape(-1, 3)
    )
    feet_quat = (
        asset.data.body_quat_w[:, body_ids, :]
        .unsqueeze(2)
        .expand(-1, -1, feet_edge_relative_pos.shape[2], -1)
        .reshape(-1, 4)
    )
    feet_edge_pos = (
        math_utils.quat_apply(feet_quat, feet_edge_relative_pos.reshape(-1, 3))
        + feet_pos
    )  # [num_envs * num_feet * num_points, 3]
...
def reward_tracking_lin_vel_x(
    env: ManagerBasedRLEnvCfg, asset_cfg: SceneEntityCfg, tracking_sigma: float = 0.25
) -> torch.Tensor:
...
def reward_action_rate(env: ManagerBasedRLEnvCfg) -> torch.Tensor:
...
def reward_dof_pos_limits(
    ...
    return torch.sum(
        ((joint_pos < soft_lower_limit) | (joint_pos > soft_upper_limit)).float(),
        dim=-1,
    )


def reward_collision(
    env: ManagerBasedRLEnvCfg, sensor_cfg: SceneEntityCfg, threshold: float = 1.0
) -> torch.Tensor:
...
def reward_feet_swing(
    ...
    return (left_swing & ~feet_contact[:, 0]).float() + (
        right_swing & ~feet_contact[:, 1]
    ).float()
...
def reward_feet_distance(  # feet are two points
    env: ManagerBasedRLEnvCfg,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    feet_distance_ref: float = 0.2,
) -> torch.Tensor:
    asset: Articulation = env.scene[asset_cfg.name]
    feet_xyz = asset.data.body_pose_w[
        :, asset_cfg.body_ids, :3
    ]  # assume body_ids has two entries for the feet
