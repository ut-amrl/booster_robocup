...
class SurvivalTime:
    ...
    def compute(self, subtask_names) -> torch.Tensor:
        ...
        results = {
            name: self.duration[i * n : (i + 1) * n].mean().item()
...
class MovementError:
    ...
    def compute(self, subtask_names) -> torch.Tensor:
        ...
        results = {
            name: movement_error[i * n : (i + 1) * n].mean().item()
            for i, name in enumerate(subtask_names)
        }
        return results
...
class Energy:
    ...
    def compute(self, subtask_names) -> torch.Tensor:
        ...
        results = {
            name: energy[i * n : (i + 1) * n].mean().item()
            for i, name in enumerate(subtask_names)
        }
        return results
...
class Smoothness:
    ...
    def __init__(self, env: ManagerBasedRLEnv) -> None:
        ...
        self.duration = torch.zeros(env.num_envs, dtype=torch.float)
        self.last_acc = torch.zeros(
            env.num_envs, env.scene["robot"].num_joints, dtype=torch.float
        )
    ...
    def compute(self, subtask_names) -> torch.Tensor:
        ...
        results = {
            name: smoothness[i * n : (i + 1) * n].mean().item()
            for i, name in enumerate(subtask_names)
        }
        return results
